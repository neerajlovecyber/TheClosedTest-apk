...
/**
 * Formats a creation date into a human-friendly relative age string (e.g. "Just now", "2h ago", "3d ago", "2w ago", "1mo ago").
 */
export const formatAppAge = (date?: string | Date | null): string => {
  if (!date) return "";
  const ms = typeof date === "string" ? new Date(date).getTime() : date instanceof Date ? date.getTime() : 0;
...
/**
 * Formats a date into a localized descriptive string (e.g. "Oct 14, 2024").
 */
export const formatFullDate = (date?: string | Date | null): string => {
  if (!date) return "";
