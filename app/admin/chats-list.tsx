...
export default function AdminChatsListScreen() {
  ...
  const nonChatUsers = query
    ? (searchedUsers || []).filter((u) => !chatUserIds.has(u.id) && (u.name?.toLowerCase().includes(query) || u.email?.toLowerCase().includes(query)))
    : [];
  ...
  const handleOpenChat = async (targetUserId: string, userName?: string) => {
    try {
      ...
      toast.error("Failed to open chat", {
        description: err.message || "Could not start conversation",
      });
    }
  };
          ...
          <CardContent className="p-4 flex-row items-center justify-between">
            <View className="flex-row items-center flex-1 mr-3">
              <View className="relative mr-3">
                {avatarUrl ? (
                  <Image source={{ uri: avatarUrl }} className="w-12 h-12 rounded-full bg-muted" />
                ) : (
                  <View className="w-12 h-12 rounded-full bg-primary/10 items-center justify-center">
                    <Icon as={UserIcon} className="text-primary size-6" />
                  </View>
                )}
                {hasUnread && <View className="absolute -top-0.5 -right-0.5 w-3.5 h-3.5 bg-red-500 rounded-full border-2 border-background" />}
              </View>
              ...
              <View className="flex-1">
                ...
                <Text className={`text-xs ${hasUnread ? "font-bold text-foreground" : "text-muted-foreground"}`} numberOfLines={1}>
    ...
    return (
      <Card className="mb-3 border-border bg-card">
        <CardContent className="p-4 flex-row items-center justify-between">
          <View className="flex-row items-center flex-1 mr-3">
            {avatarUrl ? (
              <Image source={{ uri: avatarUrl }} className="w-11 h-11 rounded-full bg-muted mr-3" />
            ) : (
              ...
              <View className="flex-row items-center gap-1.5">
                ...
                {item.isAdmin && (
                  <View className="bg-purple-100 dark:bg-purple-950 px-1.5 py-0.5 rounded">
                    <Text className="text-[9px] font-bold text-purple-600 dark:text-purple-400 uppercase">Admin</Text>
                  </View>
                )}
              </View>
          ...
          >
            ...
            <Text className="text-xs font-bold text-primary-foreground">{getOrCreateChat.isPending ? "Opening..." : "Message"}</Text>
          </TouchableOpacity>
        </CardContent>
      </Card>
    );
    ...
    <SafeAreaView className="flex-1 bg-background" edges={["top", "left", "right", "bottom"]}>
      ...
      {isChatsLoading || (query && isUsersLoading) ? (
        ...
        <View className="flex-1 items-center justify-center p-8">
          ...
          <Text className="text-lg font-bold text-foreground mb-1">{query ? "No Results Found" : "No Support Chats"}</Text>
        ...
        <SectionList
          ...
          renderSectionHeader={({ section }) => (
            <View className="px-4 py-2 bg-background/90">
              <Text className="text-xs font-bold text-muted-foreground uppercase tracking-wider">{section.title}</Text>
            </View>
          )}
