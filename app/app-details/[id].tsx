...
export default function AppDetailsScreen() {
  ...
  const handleOpenApp = async () => {
    ...
    const openPlayStore = () => {
      RNLinking.canOpenURL(marketUrl)
        .then((supported) => {
          if (supported) {
          ...
          <Card className="border-0 overflow-hidden bg-blue-950 shadow-lg">
            <CardContent className="p-0">
              <View className="p-5 flex-row items-start gap-4">
                <Image
                  source={{
                    uri: app.iconUrl || "https://images.unsplash.com/photo-1509228468518-180dd4864904?w=160&auto=format&fit=crop&q=80",
                  }}
              ...
              <CardContent className="p-5 gap-3">
                <View className="flex-row justify-between items-center">
                  ...
                  <Text className="font-bold text-primary">
                    {app.currentTesters || 0} / {app.requiredTesters} testers
                  </Text>
                </View>
                <View className="h-3 bg-secondary rounded-full overflow-hidden w-full">
                  <View
                    ...
                    style={{
                      width: `${Math.min(100, ((app.currentTesters || 0) / Math.max(1, app.requiredTesters)) * 100)}%`,
                    }}
                  />
                </View>
              </CardContent>
          ...
          <Card className="border-border bg-card shadow-sm">
            <CardContent className="p-4 flex-row items-center gap-3">
              <Image
                source={{
                  uri:
                    (isMine ? user?.avatarUrl : app.user?.avatarUrl) ||
                    app.user?.avatarUrl ||
                    user?.avatarUrl ||
              ...
              <View className="flex-1">
                ...
                <Text className="text-xs text-muted-foreground mt-0.5">{isMine ? "App Owner" : "Peer Tester"}</Text>
              </View>
              <View className="items-end bg-primary/5 px-2.5 py-1.5 rounded-xl border border-primary/10">
                ...
                <Text className="text-sm font-extrabold text-primary">★ {(isMine ? user?.reputation : app.user?.reputation) ?? user?.reputation ?? 0}</Text>
              </View>
            </CardContent>
          </Card>
        </View>
            ...
            <Card className="border-0 overflow-hidden">
              <CardContent className="p-0">
                {selectedAppData ? (
                  ...
                  >
                    <Image source={{ uri: selectedAppData.iconUrl || "https://github.com/shadcn.png" }} className="w-12 h-12 rounded-xl bg-muted" />
                    <View className="flex-1">
                      <Text className="font-bold text-base text-foreground">{selectedAppData.title}</Text>
                      <Text className="text-xs text-muted-foreground">{isLocked ? "Match Locked" : "Tap to change app"}</Text>
                    </View>
                    <Icon as={CheckCircleIcon} className="text-green-500 size-6" />
                  </TouchableOpacity>
                ) : (
                  <TouchableOpacity activeOpacity={0.7} onPress={() => setIsModalVisible(true)} className="p-6 items-center justify-center bg-secondary/20">
                    ...
                    <Text className="text-xs text-muted-foreground text-center mt-1">You need to offer one of your apps for mutual testing</Text>
                  </TouchableOpacity>
                )}
              </CardContent>
            </Card>
      ...
      <View className="p-4 border-t border-border bg-background safe-bottom">
        {isMine ? (
          <View className="flex-row gap-4">
            ...
            <Button size="lg" variant="destructive" onPress={() => setActiveAlert("delete")} className="flex-1 rounded-2xl shadow-sm" disabled={isSubmitting}>
          ...
          <Button size="lg" onPress={handleRequestSwap} className="w-full rounded-xl" disabled={isSubmitting}>
            <Text className="font-bold text-lg">{isSubmitting ? "Sending Request..." : "Start Testing Together"}</Text>
          </Button>
        )}
      </View>
                  ...
                  >
                    ...
                    <View className="flex-1">
                      ...
                      <Text className="text-muted-foreground text-sm">
                        {myapp.currentTesters} / {myapp.requiredTesters} testers
                      </Text>
                    </View>
                    {selectedMyApp === myapp.id && <Icon as={CheckCircleIcon} className="text-primary size-5" />}
                  </TouchableOpacity>
                ))}
              </ScrollView>
            )}
