...
import { BellIcon, CheckCircleIcon, ArrowLeftIcon, MessageSquareIcon, InfoIcon, AlertCircleIcon, CheckCheckIcon, Trash2Icon } from "lucide-react-native";
import {
  useNotifications,
            ...
            {notifications.map((notification) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <View className="mr-3.5 mt-0.5">{getIcon(notification.type)}</View>
                  <View className="flex-1">
                    <View className="flex-row items-center gap-2">
                      <Text className={`text-base flex-1 ${isUnread ? "font-bold text-foreground" : "font-medium text-foreground/80"}`}>
                        {notification.title}
                      </Text>
                      {isUnread && <View className="w-2.5 h-2.5 rounded-full bg-primary" />}
                    </View>
                    <Text className="text-muted-foreground text-sm mt-1 leading-relaxed">{notification.body}</Text>
