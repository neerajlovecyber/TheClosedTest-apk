...
export default function EditAppScreen() {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      try {
        const manipulated = await manipulateAsync(result.assets[0].uri, [{ resize: { width: 128, height: 128 } }], {
          compress: 0.8,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (processedImageUri) {
        try {
          iconUrl = await uploadImageToR2(processedImageUri, "icons");
        } catch (r2Err) {
    ...
    <View className="flex-1 bg-background pt-12">
      ...
      <KeyboardAwareScrollView bottomOffset={Platform.OS === "ios" ? 100 : 80} className="flex-1 p-4" contentContainerStyle={{ paddingBottom: 40 }}>
        <Card className="mb-6">
          ...
          <CardContent className="gap-4">
            <View className="mb-6 items-center">
              <TouchableOpacity onPress={pickImage} activeOpacity={0.8}>
                <View className="relative">
                  <Image
                    source={{
                      uri: processedImageUri || currentIconUrl || "https://github.com/shadcn.png",
                    }}
            ...
            <View>
              ...
              <Input nativeID="appName" value={title} onChangeText={setTitle} maxLength={30} />
            </View>
            ...
            <View>
              ...
              <Input nativeID="playUrl" value={playStoreUrl} onChangeText={setPlayStoreUrl} />
            </View>
          </CardContent>
        </Card>
        ...
        <View className="gap-3 mb-8">
          <Button size="lg" onPress={handleSubmit} disabled={isSubmitting}>
            {isSubmitting ? <ActivityIndicator color="white" size="small" /> : <Text>Save Changes</Text>}
          </Button>

          <Button size="lg" variant="destructive" onPress={() => setShowDeleteConfirm(true)} disabled={isSubmitting}>
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Are you sure you want to delete this app? This action cannot be undone.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
