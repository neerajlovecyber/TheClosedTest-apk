      ...
      <View className="flex-1 px-4 pt-4">
        ...
        {isLoading ? (
          ...
          <View className="flex-1">
            <TouchableOpacity onPress={() => router.push("/admin-chat" as any)} activeOpacity={0.7} className="mb-4">
              <Card className="border-primary/30 bg-primary/5 shadow-sm">
                <CardContent className="p-4">
                  <View className="flex-row justify-between items-start mb-2">
                    <View className="px-2 py-1 rounded-md bg-blue-100 dark:bg-blue-900/30">
                      <Text className="text-xs font-bold text-blue-700 dark:text-blue-400">{(myChat as any)?.status?.toUpperCase() || "ACTIVE SUPPORT"}</Text>
                    </View>
                    <Text className="text-xs text-muted-foreground">Live Channel</Text>
                  </View>

                  <Text className="text-lg font-bold text-foreground mb-1">Official Support Chat</Text>
            ...
            <TouchableOpacity onPress={() => router.push("/create-ticket")} className="bg-primary p-4 rounded-2xl flex-row items-center justify-center gap-2">
