...
export default function MatchDashboardScreen() {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      await Promise.all([refetchMatch(), refetchProofs(), refetchMessages()]);
  ...
  const highestProofDay = Math.max(1, ...allProofs.map((p) => p.day || 1), myLastProof?.day || 1, partnerLastProof?.day || 1);
  ...
  const selectedDayMyProof = allProofs.find((p: ProofEntity) => p.day === effectiveDay && p.uploaderId === myUserId);
  const selectedDayPartnerProof = allProofs.find((p: ProofEntity) => p.day === effectiveDay && p.uploaderId === partnerUserId);
          ...
          <View className="flex-row items-center gap-3 mb-3">
            ...
            <View className="flex-1">
              <Text className="text-xl font-bold" numberOfLines={1}>
                {partnerApp?.title || "Testing"}
              </Text>
              <Text className="text-xs text-muted-foreground" numberOfLines={1}>
                {partnerApp?.packageName}
              </Text>
            </View>
            <TouchableOpacity onPress={handleOpenApp} className="bg-primary px-3 py-2 rounded-lg">
            ...
            <Card className="bg-gradient-to-br from-amber-500/15 via-green-500/10 to-emerald-500/15 border-amber-500/40 mb-4">
              <CardContent className="p-6">
                <View className="items-center mb-6">
                  ...
                  <Text className="text-2xl font-extrabold text-amber-600 dark:text-amber-400 text-center">🎉 14-Day Testing Complete!</Text>
                  <Text className="text-sm text-muted-foreground mt-1">You've successfully completed this testing journey</Text>
                </View>
                ...
                <View className="flex-row gap-4 mb-6">
                  <View className="flex-1 bg-green-500/15 rounded-xl p-4 items-center border border-green-500/30">
                    ...
                    <Text className="text-3xl font-extrabold text-green-600">{isUser1 ? match.user1ApprovedCount : match.user2ApprovedCount}</Text>
                  ...
                  <View className="flex-1 bg-blue-500/15 rounded-xl p-4 items-center border border-blue-500/30">
                    ...
                    <Text className="text-3xl font-extrabold text-blue-600">{isUser1 ? match.user2ApprovedCount : match.user1ApprovedCount}</Text>
          ...
          <View className="px-4 mb-4">
            <Card className="bg-red-500/10 border-red-500/30">
              <CardContent className="p-5 items-center">
                ...
                <Text className="text-xl font-bold text-red-600 dark:text-red-400 text-center">Testing Match Ended</Text>
                ...
                <View className="flex-row gap-3 w-full">
                  <Button variant="outline" onPress={() => router.replace("/(tabs)/tests" as any)} className="flex-1">
                    <Text className="text-foreground font-semibold text-xs">My Tasks</Text>
                  </Button>
                  <Button onPress={() => router.replace("/(tabs)/marketplace" as any)} className="flex-1">
      ...
      {!isCancelled && (
        <View className="absolute right-6 z-50" style={{ bottom: Math.max(insets.bottom, 16) + 84 }}>
          ...
          >
            ...
            {hasUnreadChat && <View className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500 border-2 border-background shadow-sm" />}
          </TouchableOpacity>
        </View>
      )}
