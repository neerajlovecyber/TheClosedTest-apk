    ...
    <View className="flex-1 bg-background">
      ...
      <View className="bg-orange-500/10 pb-6">
        <View className="px-6 pt-14 pb-2 flex-row items-center gap-4">
          <TouchableOpacity onPress={() => router.back()} className="w-10 h-10 rounded-full bg-background/80 items-center justify-center">
                    ...
                    >
                      <View
                        className={`w-8 h-8 rounded-lg items-center justify-center ${
                          rank === 1 ? "bg-yellow-500" : rank === 2 ? "bg-gray-400" : rank === 3 ? "bg-orange-600" : "bg-muted"
                        }`}
                      >
                        <Text className={`font-bold text-sm ${rank <= 3 ? "text-white" : "text-muted-foreground"}`}>{rank}</Text>
                      </View>

                      {item.avatarUrl && !item.avatarUrl.includes("shadcn.png") ? (
                        <Image source={{ uri: item.avatarUrl }} style={{ width: 40, height: 40, borderRadius: 20 }} contentFit="cover" />
                      ) : (
                        <View className="w-10 h-10 rounded-full bg-primary/10 items-center justify-center">
                          <Text className="text-sm font-bold text-primary">{item.name?.substring(0, 2).toUpperCase() || "??"}</Text>
                        </View>
                      )}
                      ...
                      <View className="flex-1">
                        ...
                        <Text className="text-xs text-muted-foreground">{item.completedMatchesCount || 0} apps tested</Text>
                      </View>

                      <View className="items-end">
                        <Text className="font-bold text-orange-500 text-lg">{item.reputation}</Text>
