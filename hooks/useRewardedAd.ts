...
const AD_UNIT_ID_APP_SLOTS = __DEV__ ? TestIds.REWARDED : "ca-app-pub-3238435978294704/2016482105";
...
export function useRewardedAd(adUnitId: string = AD_UNIT_ID_APP_SLOTS) {
  ...
  const showAd = useCallback((): Promise<boolean> => {
    return new Promise((resolve) => {
      if (!rewarded || !loaded) {
        toast.info("Ad Not Ready", {
          description: "Please wait for the ad to load and try again.",
        });
        resolve(false);
        return;
      }
