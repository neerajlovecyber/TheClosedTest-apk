...
async fn get_clerk_jwks(state: &AppState) -> Option<JwkSet> {
    ...
    state.jwks_cache.insert(domain.clone(), jwks.clone()).await;
    Some(jwks)
}
...
pub async fn verify_token_payload(raw_token: &str, state: &AppState) -> Option<TokenPayload> {
    ...
    // 3. In test environment or if Clerk secret key is configured for symmetric testing
    ...
    // 4. Cryptographically verify signature against Clerk's remote JWKS (RS256)
    if let Some(jwks) = get_clerk_jwks(state).await {
    ...
    // 5. Fallback for offline resilience if JWKS fetch failed but token is structurally valid
