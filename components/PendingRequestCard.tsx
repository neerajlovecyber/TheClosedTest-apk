      ...
      <CardContent className="p-3">
        ...
        <View className="bg-secondary/30 rounded-md p-2 mb-3 border border-secondary/50">
          <View className="flex-row items-center justify-between">
            ...
            <View className="flex-1">
              ...
              {offeredApp && (
                <Text
                  className={`text-[10px] ${offeredApp.currentTesters && offeredApp.requiredTesters && offeredApp.currentTesters >= offeredApp.requiredTesters ? "text-destructive font-bold" : "text-muted-foreground"}`}
                >
            ...
            <View className="flex-1 items-end">
              ...
              {myApp && (
                <Text
                  className={`text-[10px] ${myApp.currentTesters && myApp.requiredTesters && myApp.currentTesters >= myApp.requiredTesters ? "text-destructive font-bold" : "text-muted-foreground"}`}
                >
        ...
        {((offeredApp?.currentTesters !== undefined && offeredApp?.requiredTesters !== undefined && offeredApp.currentTesters >= offeredApp.requiredTesters) ||
          ...
          <View className="bg-destructive/10 p-1.5 rounded-md mb-2 items-center">
            <Text className="text-[10px] font-bold text-destructive">Cannot Accept - Capacity Reached</Text>
          </View>
        )}
        ...
        <View className="flex-row gap-2">
          ...
          <Button
            ...
            className={`flex-1 h-8 px-0 shadow-sm ${
              (offeredApp?.currentTesters && offeredApp?.requiredTesters && offeredApp.currentTesters >= offeredApp.requiredTesters) ||
