...
export function MatchChat({ visible, onClose, matchId, partnerName, onReport, currentUserId, partnerLastRead }: MatchChatProps) {
  ...
  const renderMessage = ({ item }: { item: any }) => {
    ...
    const isSeen = Boolean(partnerLastRead && new Date(partnerLastRead).getTime() >= new Date(item.sentAt).getTime());

    return (
      <View className={`flex-row ${isMe ? "justify-end" : "justify-start"} mb-3 px-4`}>
        <View style={{ maxWidth: "80%" }} className={`px-4 py-2 rounded-2xl ${isMe ? "bg-primary rounded-tr-none" : "bg-secondary rounded-tl-none"}`}>
          <Text className={`${isMe ? "text-primary-foreground font-medium" : "text-foreground"}`}>{item.content}</Text>
          <View className={`flex-row items-center gap-1 mt-1 ${isMe ? "justify-end" : "justify-start"}`}>
            ...
            {isMe && <Icon as={isSeen ? CheckCheckIcon : CheckIcon} className={`size-3.5 ${isSeen ? "text-sky-300" : "text-primary-foreground/60"}`} />}
          </View>
        </View>
      </View>
    );
    ...
    <Modal visible={visible} animationType="slide" transparent={true} statusBarTranslucent={true} onRequestClose={onClose}>
      <KeyboardAvoidingView behavior="padding" className="flex-1">
        <View className="flex-1 justify-end">
          ...
          <View style={{ height: SCREEN_HEIGHT * 0.75 }} className="bg-background rounded-t-[32px] overflow-hidden shadow-2xl border-t border-border">
