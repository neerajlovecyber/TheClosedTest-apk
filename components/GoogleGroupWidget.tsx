...
export function GoogleGroupWidget({ className }: { className?: string }) {
  ...
  React.useEffect(() => {
    const subscription = AppState.addEventListener("change", (nextAppState) => {
      if (appState.current.match(/inactive|background/) && nextAppState === "active" && hasClickedLink) {
        if (isGroupMember) {
          setHasClickedLink(false);
        } else {
    ...
    return (
      <Card className={cn("border-amber-200 bg-amber-50 dark:bg-amber-900/10 dark:border-amber-900/50 overflow-hidden", className)}>
        ...
        <CardContent className="p-4">
          ...
          <Button size="sm" className="bg-amber-600 dark:bg-amber-700 shadow-sm shadow-amber-900/20" onPress={handleJoinGroup}>
      ...
      <AlertDialog open={showJoinConfirm} onOpenChange={setShowJoinConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Have you successfully joined the Google Group?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
              onPress={() => {
