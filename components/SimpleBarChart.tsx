...
export function SimpleBarChart({ data, selectedDate, onSelectDate, barColor = "bg-primary" }: SimpleBarChartProps) {
  ...
  return (
    <View className="my-6 bg-card p-4 rounded-xl border border-border shadow-sm">
      <Text className="text-sm font-semibold mb-8 text-muted-foreground">Last 7 Days Activity</Text>
      <View className="flex-row items-end justify-between h-32 px-2">
        {data.map((d) => {
          ...
          return (
            <TouchableOpacity key={d.date} onPress={() => onSelectDate?.(isSelected ? null : d.date)} disabled={!onSelectDate} className="items-center w-8">
              <View className="w-full items-center justify-end h-full">
                <View className="relative w-4" style={{ height: `${Math.max(heightPercentage, 10)}%` }}>
                  {d.value > 0 && <Text className="absolute -top-5 left-0 right-0 text-center text-[10px] text-muted-foreground">{d.value}</Text>}
                  <View className={`w-full h-full rounded-t-sm ${isSelected ? barColor : `${barColor}`}`} />
                </View>
              </View>
              <Text className={`text-[10px] mt-2 ${isSelected ? "text-primary font-bold" : "text-muted-foreground"}`}>{d.label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
